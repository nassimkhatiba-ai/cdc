#!/usr/bin/env node
// mcp-call.js --- stdio MCP bridge for CDC scripts (package: turtle).
//
// KEY PROPERTIES:
...
//         CDC_BRIDGE_MAX_BYTES (default 2500: truncate large CLI stdout; scripts stay full)
...
const BRIDGE_MAX_BYTES = parseInt(process.env.CDC_BRIDGE_MAX_BYTES || '2500', 10);

function wantPretty(argv) {
  return process.env.CDC_BRIDGE_PRETTY === '1' || argv.includes('--pretty');
}

/** Compact one-line JSON by default; soft byte cap on CLI only (library path stays full). */
function printJson(value, pretty) {
  let s = pretty ? JSON.stringify(value, null, 2) : JSON.stringify(value);
  if (typeof value === 'string') s = value;
  if (BRIDGE_MAX_BYTES > 0 && Buffer.byteLength(s) > BRIDGE_MAX_BYTES) {
    let summary;
    if (Array.isArray(value)) {
      summary = {
        _truncated: true,
        n: value.length,
        keys: value[0] && typeof value[0] === 'object' ? Object.keys(value[0]).slice(0, 12) : [],
      ...
      summary = {
        ...
        preview: s.slice(0, Math.max(0, BRIDGE_MAX_BYTES - 120)),
      };
    } else {
      summary = {
        _truncated: true,
        bytes: Buffer.byteLength(s),
        max: BRIDGE_MAX_BYTES,
        preview: s.slice(0, Math.max(0, BRIDGE_MAX_BYTES - 80)),
      };
    }
    // re-cap summary if still huge
    let out = JSON.stringify(summary);
    if (Buffer.byteLength(out) > BRIDGE_MAX_BYTES) {
      out = JSON.stringify({
        _truncated: true,
        bytes: Buffer.byteLength(s),
        max: BRIDGE_MAX_BYTES,
...
function resolveServer() {
  ...
  const command = process.env.CDC_MCP_COMMAND || man.command || "node";
  const args = process.env.CDC_MCP_ARGS
    ? JSON.parse(process.env.CDC_MCP_ARGS)
    : (man.args && man.args.length ? man.args : ["/Users/nesbes/mcp-a;t/implementer/turtle-canvas/bin/turtle-canvas-mcp.js"]);
  if (!command) {
    throw new Error('Set CDC_MCP_COMMAND or re-run: cdc from-mcp --probe <cmd> --name turtle');
  }
  return { command, args };
}
...
function socketPath() {
  ...
  return path.join(os.tmpdir(), 'cdc-turtle-' + h + '.sock');
}
  ...
  (async () => {
    ...
    try {
      if (argv[0] === '__daemon__') {
        ...
        s.close();
        printJson(tools.map((t) => t.name), wantPretty(argv));
      } else if (argv[0] === '--batch') {
        const calls = JSON.parse(argv[1] || '[]');
        const results = await callTools(calls);
        printJson(results, wantPretty(argv));
      } else {
        const toolArgv = argv.filter((a) => a !== '--pretty');
        const r = await callTool(toolArgv[0], toolArgv[1] ? JSON.parse(toolArgv[1]) : {});
        printJson(typeof r === 'string' ? r : r, wantPretty(argv));
      }
    } catch (e) {
