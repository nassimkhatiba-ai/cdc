---
name: notion-cdc
description: Fast CDC for notion (24 tools). Multi-hop: ONE node openSession script; max 1 shell. No MCP schemas. Use notion-cdc skill.
---
...
(async () => {
  ...
  const rows = await callPaged(s, 'API-post-search', {}); // ALL pages
  const one = await s.call('API-get-user', {}); // join/filter in-process
...
3. Paginate with callPaged(`API-post-search`) — never page 1 only. Prefer list/search tools for callPaged, never patch/update/create.
