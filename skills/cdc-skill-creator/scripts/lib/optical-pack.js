...
function chooseSkillMode({ force, estVisionTokens, textEquivalentTokens, toolCount }) {
  const margin = parseFloat(process.env.CDC_OPTICAL_MARGIN || '0.85');
  const minTools = parseInt(process.env.CDC_OPTICAL_MIN_TOOLS || '8', 10);
  ...
  const visionCost = estVisionTokens + pointerTokens;
  if (visionCost < textEquivalentTokens * margin) {
    return {
      mode: 'image',
      reason: `optical ${estVisionTokens}+${pointerTokens} ptr < ${textEquivalentTokens} text-equiv × ${margin}`,
    };
  }
  return {
    mode: 'text',
    reason: `optical ${estVisionTokens}+${pointerTokens} ptr not < ${textEquivalentTokens} text-equiv × ${margin}`,
  };
}
