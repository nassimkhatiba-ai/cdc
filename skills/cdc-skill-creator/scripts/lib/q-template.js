// Builds q.js — the tiny data toolkit shipped inside direct-fs CDC skills.
//
// Purpose: shrink agent-written scripts from ~40 generated lines to ~8
// (generation time is the dominant CDC latency), and turn the observed
// failure modes into hard errors instead of silent wrong answers:
//   - q.index() coerces join keys to String on both sides (no silent
//     number-vs-string join misses)
//   - q.assertNonEmpty() makes a zero/empty metric throw with a hint
//     instead of printing 0
function buildQjs(root) {
  const rootExpr = root
    ? `process.env.CDC_FS_ROOT || ${JSON.stringify(root)}`
    : `process.env.CDC_FS_ROOT || process.cwd()`;
