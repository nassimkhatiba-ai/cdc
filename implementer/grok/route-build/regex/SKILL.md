---
name: regex-cdc
description: Fast CDC for regex (4 tools). ONE shell: node mcp-call.js --batch '[{tool,args}...]'. No MCP schemas. Use regex-cdc skill.
---
...
```bash
node '__SKILL_DIR__/mcp-call.js' --batch '[{"tool":"match","args":{}}]'
```

Single: `node '__SKILL_DIR__/mcp-call.js' match '{}'`
...
### find
find_all(pattern*, s*) — Find all matches
### misc
match(pattern*, s*) — Test regex
replace(pattern*, s*, with*) — Replace all
count(pattern*, s*) — Count matches
