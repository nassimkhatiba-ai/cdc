#!/usr/bin/env node
// mcp-call.js --- stdio MCP bridge for CDC scripts (package: regex).
//
// KEY PROPERTIES:
...
function resolveServer() {
  ...
  const command = process.env.CDC_MCP_COMMAND || man.command || "implementer/mega40/homes/regex/regex-mcp.sh";
  const args = process.env.CDC_MCP_ARGS
    ? JSON.parse(process.env.CDC_MCP_ARGS)
    : (man.args && man.args.length ? man.args : []);
  if (!command) {
    throw new Error('Set CDC_MCP_COMMAND or re-run: cdc from-mcp --probe <cmd> --name regex');
  }
  return { command, args };
}
...
function socketPath() {
  ...
  return path.join(os.tmpdir(), 'cdc-regex-' + h + '.sock');
}
