---
name: everything-cdc
description: Fast CDC for everything (13 tools). Multi-hop: ONE node openSession script; max 1 shell. No MCP schemas. Use everything-cdc skill.
---
...
(async () => {
  ...
  const one = await s.call('echo', {}); // join/filter in-process
