---
name: mathstat-cdc
description: Fast CDC for mathstat (5 tools). ONE shell: node mcp-call.js --batch '[{tool,args}...]'. No MCP schemas. Use mathstat-cdc skill.
---
...
```bash
node '__SKILL_DIR__/mcp-call.js' --batch '[{"tool":"sum","args":{}}]'
```

Single: `node '__SKILL_DIR__/mcp-call.js' sum '{}'`
...
mean(nums*:[]) — Mean
median(nums*:[]) — Median
stdev(nums*:[]) — Population stdev
percentile(nums*:[], p*:number) — Percentile p in 0..100
