---
name: books-cdc
description: Fast CDC for books (5 tools). Multi-hop: ONE node openSession script; max 1 shell. No MCP schemas. Use books-cdc skill.
---
...
(async () => {
  ...
  const rows = await callPaged(s, 'list_books', {}); // ALL pages
  const one = await s.call('list_books', {}); // join/filter in-process
...
3. Paginate with callPaged(`list_books`) — never page 1 only. Prefer list/search tools for callPaged, never patch/update/create.
...
List tools: list_books

list_books get_book search count_by_genre top_price
