---
name: mockapi-cdc
description: Fast CDC for mockapi (5 tools). Multi-hop: ONE node openSession script; max 1 shell. No MCP schemas. Use mockapi-cdc skill.
---
...
(async () => {
  ...
  const rows = await callPaged(s, 'list_orders', {}); // ALL pages
  const one = await s.call('list_products', {}); // join/filter in-process
...
3. Paginate with callPaged(`list_orders`) — never page 1 only. Prefer list/search tools for callPaged, never patch/update/create.
...
List tools: list_products list_orders list_users

list_products list_orders list_users get_product get_spec
