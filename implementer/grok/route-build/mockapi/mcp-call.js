#!/usr/bin/env node
// mcp-call.js --- stdio MCP bridge for CDC scripts (package: mockapi).
//
// KEY PROPERTIES:
...
function resolveServer() {
  ...
  const command = process.env.CDC_MCP_COMMAND || man.command || "implementer/mega40/homes/mockapi/mockapi-mcp.sh";
  const args = process.env.CDC_MCP_ARGS
    ? JSON.parse(process.env.CDC_MCP_ARGS)
    : (man.args && man.args.length ? man.args : []);
  if (!command) {
    throw new Error('Set CDC_MCP_COMMAND or re-run: cdc from-mcp --probe <cmd> --name mockapi');
  }
  return { command, args };
}
...
function socketPath() {
  ...
  return path.join(os.tmpdir(), 'cdc-mockapi-' + h + '.sock');
}
